...
plt.rcParams.update({'font.size': 11, 'axes.labelsize': 12, 'axes.titlesize': 13, 'xtick.labelsize': 10, 'ytick.labelsize': 10, 'legend.fontsize': 10, 'figure.dpi': 150, 'savefig.dpi': 300, 'savefig.bbox': 'tight'})
...
def stats(arr):
    if len(arr) == 0:
        return (0, 0)
    mean = np.mean(arr)
    std = np.std(arr, ddof=1) / np.sqrt(len(arr))
    return (mean, std)
print('Loading data...')
...
for name, entries in interventions.items():
    ...
    intervention_metrics[name] = {'accuracy': acc, 'match_rate': match, 'flip_rate': flip, 'mwc': mwc, 'mww': mww}
print('\nGenerating Figure 1: Baseline Accuracy Comparison...')
fig, ax = plt.subplots(figsize=(12, 6))
conditions = [('Model A\n(Qwen)', model_a_acc, 'tab:blue'), ('Model B No CoT\n(Llama)', model_b_no_cot_acc, 'tab:orange'), ('Model B + CoT\n(Baseline)', model_b_with_cot_acc, 'tab:green')]
key_interventions = [('truncate_last_k5', 'Trunc Last k=5'), ('truncate_percent_p0.5', 'Trunc 50%'), ('filler_replacement_p0.5', 'Filler 50%'), ('error_injection', 'Error Inject')]
...
for bar, mean, std in zip(bars, means, stds):
    ax.annotate(f'{mean:.1f}', xy=(bar.get_x() + bar.get_width() / 2, mean + std + 2), ha='center', fontsize=9)
plt.tight_layout()
...
ax = axes[0, 0]
for prefix, color, marker, label in [('truncate_first', 'tab:blue', 'o', 'Truncate First'), ('truncate_last', 'tab:orange', 's', 'Truncate Last'), ('truncate_contiguous', 'tab:green', '^', 'Truncate Contiguous')]:
    ks, means, stds = ([], [], [])
    for k in [1, 2, 3, 5]:
        key = f'{prefix}_k{k}'
        if key in intervention_metrics:
            ...
            stds.append(100 * s)
    if ks:
        ax.errorbar(ks, means, yerr=stds, marker=marker, color=color, label=label, linewidth=2, markersize=8, capsize=4)
...
for p in [0.1, 0.2, 0.3, 0.5]:
    ...
    if key in intervention_metrics:
        ...
        stds.append(100 * s)
if ps:
    ax.errorbar(ps, means, yerr=stds, marker='o', color='tab:blue', label='Filler Replace', linewidth=2, markersize=8, capsize=4)
ax.axhline(y=100 * baseline_match_mean, color='gray', linestyle='--', alpha=0.7, label='Baseline')
...
for p in [0.1, 0.2, 0.3, 0.5]:
    ...
    if key in intervention_metrics:
        ...
        stds.append(100 * s)
if ps:
    ax.errorbar(ps, means, yerr=stds, marker='s', color='tab:green', label='Filler Replace', linewidth=2, markersize=8, capsize=4)
ax.axhline(y=100 * baseline_mwc_mean, color='gray', linestyle='--', alpha=0.7, label='Baseline')
ax.set_xlabel('Percentage of Sentences Replaced (%)')
ax.set_ylabel('MWC (%)')
...
for p in [0.1, 0.2, 0.3, 0.5]:
    ...
    if key in intervention_metrics:
        ...
        stds.append(100 * s)
if ps:
    ax.errorbar(ps, means, yerr=stds, marker='^', color='tab:orange', label='Filler Replace', linewidth=2, markersize=8, capsize=4)
ax.axhline(y=100 * baseline_mww_mean, color='gray', linestyle='--', alpha=0.7, label='Baseline')
ax.set_xlabel('Percentage of Sentences Replaced (%)')
ax.set_ylabel('MWW (%)')
...
for p in [0.1, 0.2, 0.3, 0.5]:
    ...
    if key in intervention_metrics:
        ...
        stds.append(100 * s)
if ps:
    ax.errorbar(ps, means, yerr=stds, marker='D', color='tab:red', label='Filler Replace', linewidth=2, markersize=8, capsize=4)
ax.axhline(y=100 * baseline_flip_mean, color='gray', linestyle='--', alpha=0.7, label='Baseline')
ax.set_xlabel('Percentage of Sentences Replaced (%)')
ax.set_ylabel('Flip Rate (%)')
...
all_results = []
all_results.append({'name': 'Model A (Qwen)', 'category': 'Baseline', 'omr_mean': '-', 'omr_se': '-', 'mwc_mean': '-', 'mwc_se': '-', 'mww_mean': '-', 'mww_se': '-', 'flip_rate_mean': '-', 'flip_rate_se': '-', 'n': len(model_a_acc)})
all_results.append({'name': 'Model B No CoT', 'category': 'Baseline', 'omr_mean': '-', 'omr_se': '-', 'mwc_mean': '-', 'mwc_se': '-', 'mww_mean': '-', 'mww_se': '-', 'flip_rate_mean': '-', 'flip_rate_se': '-', 'n': len(model_b_no_cot_acc)})
all_results.append({'name': 'Model B + CoT (Baseline)', 'category': 'Baseline', 'omr_mean': 100 * stats(model_b_with_cot_match)[0], 'omr_se': 100 * stats(model_b_with_cot_match)[1], 'mwc_mean': 100 * stats(model_b_with_cot_mwc)[0], 'mwc_se': 100 * stats(model_b_with_cot_mwc)[1], 'mww_mean': 100 * stats(model_b_with_cot_mww)[0], 'mww_se': 100 * stats(model_b_with_cot_mww)[1], 'flip_rate_mean': 100 * stats(1 - model_b_with_cot_match)[0], 'flip_rate_se': 100 * stats(1 - model_b_with_cot_match)[1], 'n': len(model_b_with_cot_acc)})
intervention_order = [('truncate_first_k1', 'Truncate First', 'k=1'), ('truncate_first_k2', 'Truncate First', 'k=2'), ('truncate_first_k3', 'Truncate First', 'k=3'), ('truncate_first_k5', 'Truncate First', 'k=5'), ('truncate_last_k1', 'Truncate Last', 'k=1'), ('truncate_last_k2', 'Truncate Last', 'k=2'), ('truncate_last_k3', 'Truncate Last', 'k=3'), ('truncate_last_k5', 'Truncate Last', 'k=5'), ('truncate_contiguous_k1', 'Truncate Contiguous', 'k=1'), ('truncate_contiguous_k2', 'Truncate Contiguous', 'k=2'), ('truncate_contiguous_k3', 'Truncate Contiguous', 'k=3'), ('truncate_contiguous_k5', 'Truncate Contiguous', 'k=5'), ('truncate_percent_p0.1', 'Truncate Random', '10%'), ('truncate_percent_p0.2', 'Truncate Random', '20%'), ('truncate_percent_p0.3', 'Truncate Random', '30%'), ('truncate_percent_p0.5', 'Truncate Random', '50%'), ('filler_replacement_p0.1', 'Filler Replace', '10%'), ('filler_replacement_p0.2', 'Filler Replace', '20%'), ('filler_replacement_p0.3', 'Filler Replace', '30%'), ('filler_replacement_p0.5', 'Filler Replace', '50%'), ('error_injection', 'Error Injection', '-')]
for key, category, param in intervention_order:
    if key in intervention_metrics:
        m = intervention_metrics[key]
        all_results.append({'name': f'{category} {param}', 'category': category, 'omr_mean': 100 * stats(m['match_rate'])[0], 'omr_se': 100 * stats(m['match_rate'])[1], 'mwc_mean': 100 * stats(m['mwc'])[0], 'mwc_se': 100 * stats(m['mwc'])[1], 'mww_mean': 100 * stats(m['mww'])[0], 'mww_se': 100 * stats(m['mww'])[1], 'flip_rate_mean': 100 * stats(m['flip_rate'])[0], 'flip_rate_se': 100 * stats(m['flip_rate'])[1], 'n': len(m['accuracy'])})
print('\n' + '=' * 160)
print('COMPLETE RESULTS TABLE')
print('=' * 160)
print(f"{'Condition':<30} {'OMR':>12} {'MWC':>12} {'MWW':>12} {'Flip Rate':>12} {'n':>8}")
print('-' * 160)
for r in all_results:
    omr_str = f"{r['omr_mean']:.1f} ± {r['omr_se']:.1f}" if isinstance(r['omr_se'], float) else str(r['omr_mean'])
    mwc_str = f"{r['mwc_mean']:.1f} ± {r['mwc_se']:.1f}" if isinstance(r['mwc_se'], float) else str(r['mwc_mean'])
    mww_str = f"{r['mww_mean']:.1f} ± {r['mww_se']:.1f}" if isinstance(r['mww_se'], float) else str(r['mww_mean'])
    flip_str = f"{r['flip_rate_mean']:.1f} ± {r['flip_rate_se']:.1f}" if isinstance(r['flip_rate_se'], float) else str(r['flip_rate_mean'])
    print(f"{r['name']:<30} {omr_str:>12} {mwc_str:>12} {mww_str:>12} {flip_str:>12} {r['n']:>8}")
...
for r in all_results:
    ...
    if cat != current_category:
        ...
        cat_display = ''
    omr_str = f"${r['omr_mean']:.1f} \\pm {r['omr_se']:.1f}$" if isinstance(r['omr_se'], float) else '-'
    mwc_str = f"${r['mwc_mean']:.1f} \\pm {r['mwc_se']:.1f}$" if isinstance(r['mwc_se'], float) else '-'
    mww_str = f"${r['mww_mean']:.1f} \\pm {r['mww_se']:.1f}$" if isinstance(r['mww_se'], float) else '-'
...
with open(OUTPUT_DIR / 'complete_results.csv', 'w', newline='') as f:
    writer = csv.DictWriter(f, fieldnames=['name', 'category', 'omr_mean', 'omr_se', 'mwc_mean', 'mwc_se', 'mww_mean', 'mww_se', 'flip_rate_mean', 'flip_rate_se', 'n'])
    writer.writeheader()

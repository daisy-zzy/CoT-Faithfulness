...
from .generation import run_qwen_stage, run_llama_stage
from .eval import (
    compute_agreement,
...
def full_pipeline(
    ...
    # 3. Stage 2: run Model B (Llama) with problem + CoT_A
    llama_engine = VLLMEngine(llama_name)
    final_results = run_llama_stage(llama_engine, qwen_results)
    save_jsonl(Path(out_dir) / "stage2_final.jsonl", final_results)
    ...
    # Optional: keep the old single-number agreement for compatibility.

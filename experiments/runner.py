...
from utils.statistics import MetricStats, ExperimentResults, compute_accuracy_from_rollouts, compute_omr_from_rollouts, compute_mwc_from_rollouts, compute_mww_from_rollouts, compute_flip_rate_from_rollouts
...
class ExperimentRunner:
    ...
    def __init__(self, config, dry_run=False):
        ...
        self.dataset = None
        self.model_a_results = None
        self.fixed_cot_indices = None
        self.model_b_no_cot_results = None
        self.model_b_follow_cot_results = None
        self._wikipedia_sentences = None
    ...
    def _get_wikipedia_sentences(self):
        if self._wikipedia_sentences is None:
            ...
            self._wikipedia_sentences = load_wikipedia_sentences(num_articles=self.config.wikipedia_num_articles, subset=self.config.wikipedia_subset, cache_path=cache_path, seed=self.config.random_seed)
    ...
    def _create_engine(self, model_config):
        ...
        return VLLMEngine(model_config.name, max_num_seqs=self.config.inference.max_num_seqs, max_model_len=self.config.inference.max_model_len, enable_chunked_prefill=self.config.inference.enable_chunked_prefill)

    def _generate_with_rollouts(self, engine, prompts, n_rollouts, temperature, max_tokens, model_name, desc='Generating'):
        n_prompts = len(prompts)
        if self.dry_run:
            return [[f'[DRY RUN] Response {j} for prompt {i}' for j in range(n_rollouts)] for i in range(n_prompts)]
        flat_prompts = []
        flat_seeds = []
        flat_indices = []
        ...
        print(f'{desc}: {n_prompts} prompts × {n_rollouts} rollouts = {len(flat_prompts)} total generations')
        if self.config.inference.use_ray and self.config.inference.num_gpus > 1:
            flat_outputs = self._generate_with_ray(flat_prompts, flat_seeds, temperature, max_tokens, model_name)
        else:
            flat_outputs = engine.generate_batched(flat_prompts, temperature=temperature, max_tokens=max_tokens, seeds=flat_seeds, batch_size=self.config.inference.batch_size, show_progress=True)
        results = []
    ...
    def _generate_with_ray(self, prompts, seeds, temperature, max_tokens, model_name):
        from utils.ray_inference import RayVLLMInference
        engine = RayVLLMInference(model_name=model_name, num_gpus=self.config.inference.num_gpus, max_model_len=self.config.inference.max_model_len, max_tokens=max_tokens, temperature=temperature, batch_size=self.config.inference.batch_size, max_num_batched_tokens=self.config.inference.max_num_batched_tokens, enable_chunked_prefill=self.config.inference.enable_chunked_prefill)
    ...
    def run_model_a_baseline(self):
        ...
        prompts = [PromptTemplates.qwen_cot_prompt(item['problem']) for item in self.dataset]
        all_rollout_outputs = self._generate_with_rollouts(engine=engine, prompts=prompts, n_rollouts=self.config.n_rollouts, temperature=self.config.model_a.temperature, max_tokens=self.config.model_a.max_tokens, model_name=self.config.model_a.name, desc='Model A generation')
        results = []
        for idx, item in enumerate(tqdm(self.dataset, desc='Processing Model A outputs')):
            ...
            for output in all_rollout_outputs[idx]:
                ...
                rollout_outputs.append({'output': output, 'cot': cot, 'answer': answer, 'answer_normalized': answer_normalized})
            results.append({'id': item.get('id'), 'problem': item['problem'], 'ground_truth': gt, 'ground_truth_normalized': gt_normalized, 'rollouts': rollout_outputs})
    ...
    def _fix_cot_indices(self):
        self.fixed_cot_indices = [self.rng.randint(0, self.config.n_rollouts - 1) for _ in range(len(self.model_a_results))]
    ...
    def get_fixed_cot_and_answer(self, example_idx):
    ...
    def run_model_b_no_cot_baseline(self):
        ...
        prompts = [PromptTemplates.llama_no_cot_prompt(item['problem']) for item in self.model_a_results]
        all_rollout_outputs = self._generate_with_rollouts(engine=engine, prompts=prompts, n_rollouts=self.config.n_rollouts, temperature=self.config.model_b.temperature, max_tokens=self.config.model_b.max_tokens, model_name=self.config.model_b.name, desc='Model B No-CoT generation')
        results = []
        for idx, item in enumerate(tqdm(self.model_a_results, desc='Processing Model B No-CoT outputs')):
            rollout_outputs = []
            for output in all_rollout_outputs[idx]:
                _, answer = extract_cot_and_answer(output)
                answer_normalized = normalize_answer(answer) if answer else None
                rollout_outputs.append({'output': output, 'answer': answer, 'answer_normalized': answer_normalized})
            results.append({'id': item.get('id'), 'problem': item['problem'], 'ground_truth': item['ground_truth'], 'ground_truth_normalized': item['ground_truth_normalized'], 'rollouts': rollout_outputs})
        self._save_jsonl(results, output_path)
    ...
    def run_model_b_follow_cot_baseline(self):
        ...
        for idx, item in enumerate(self.model_a_results):
            ...
            prompts.append(PromptTemplates.llama_from_cot_prompt(item['problem'], fixed_cot or ''))
        all_rollout_outputs = self._generate_with_rollouts(engine=engine, prompts=prompts, n_rollouts=self.config.n_rollouts, temperature=self.config.model_b.temperature, max_tokens=self.config.model_b.max_tokens, model_name=self.config.model_b.name, desc='Model B Follow-CoT generation')
        results = []
        for idx, item in enumerate(tqdm(self.model_a_results, desc='Processing Model B Follow-CoT outputs')):
            rollout_outputs = []
            for output in all_rollout_outputs[idx]:
                _, answer = extract_cot_and_answer(output)
                answer_normalized = normalize_answer(answer) if answer else None
                rollout_outputs.append({'output': output, 'answer': answer, 'answer_normalized': answer_normalized})
            results.append({'id': item.get('id'), 'problem': item['problem'], 'ground_truth': item['ground_truth'], 'ground_truth_normalized': item['ground_truth_normalized'], 'fixed_cot': fixed_cots[idx], 'fixed_answer_a': fixed_answers[idx], 'rollouts': rollout_outputs})
        self._save_jsonl(results, output_path)
    ...
    def run_intervention(self, intervention_config):
        print('\n' + '-' * 60)
        ...
        intervention_fn = get_intervention_fn(intervention_config.name, intervention_config.params, self.rng, wiki_sentences)
        modified_cots = []
        fixed_cots = []
        fixed_answers = []
        ...
        prompts = [PromptTemplates.llama_from_cot_prompt(item['problem'], modified_cot) for item, modified_cot in zip(self.model_a_results, modified_cots)]
        engine = None
        if not (self.config.inference.use_ray and self.config.inference.num_gpus > 1):
            engine = self._create_engine(self.config.model_b)
        all_rollout_outputs = self._generate_with_rollouts(engine=engine, prompts=prompts, n_rollouts=self.config.n_rollouts, temperature=self.config.model_b.temperature, max_tokens=self.config.model_b.max_tokens, model_name=self.config.model_b.name, desc=f'Intervention {intervention_config.run_id}')
        results = []
        for idx, item in enumerate(tqdm(self.model_a_results, desc=f'Processing {intervention_config.run_id} outputs')):
            rollout_outputs = []
            for output in all_rollout_outputs[idx]:
                _, answer = extract_cot_and_answer(output)
                answer_normalized = normalize_answer(answer) if answer else None
                rollout_outputs.append({'output': output, 'answer': answer, 'answer_normalized': answer_normalized})
            results.append({'id': item.get('id'), 'problem': item['problem'], 'ground_truth': item['ground_truth'], 'ground_truth_normalized': item['ground_truth_normalized'], 'original_cot': fixed_cots[idx], 'modified_cot': modified_cots[idx], 'fixed_answer_a': fixed_answers[idx], 'rollouts': rollout_outputs})
        self._save_jsonl(results, output_path)
        if engine is not None:
            del engine
        self._cleanup_gpu()
    ...
    def _run_error_injection_intervention(self, output_path):
        from utils.prompts import PromptTemplates
        ...
        fixed_answers = [self.get_fixed_cot_and_answer(i)[1] for i in range(len(self.model_a_results))]
        ...
        if qwen_engine is not None:
            del qwen_engine
        self._cleanup_gpu()
        prompts = [PromptTemplates.llama_from_cot_prompt(problem, modified_cot) for problem, modified_cot in zip(problems, modified_cots)]
        print('Running Model B on error-injected CoTs...')
        llama_engine = None
        if not (self.config.inference.use_ray and self.config.inference.num_gpus > 1):
            llama_engine = self._create_engine(self.config.model_b)
        all_rollout_outputs = self._generate_with_rollouts(engine=llama_engine, prompts=prompts, n_rollouts=self.config.n_rollouts, temperature=self.config.model_b.temperature, max_tokens=self.config.model_b.max_tokens, model_name=self.config.model_b.name, desc='Model B (error injection)')
        results = []
        for idx, item in enumerate(tqdm(self.model_a_results, desc='Processing error injection outputs')):
            rollout_outputs = []
            for output in all_rollout_outputs[idx]:
                _, answer = extract_cot_and_answer(output)
                answer_normalized = normalize_answer(answer) if answer else None
    ...
    def compute_all_metrics(self):
        ...
        def extract_rollout_preds(results):
            return [[r.get('answer_normalized', '') for r in item.get('rollouts', [])] for item in results]
        ground_truths = [item['ground_truth_normalized'] for item in self.model_a_results]
        model_a_rollouts = [[r.get('answer_normalized', '') for r in item.get('rollouts', [])] for item in self.model_a_results]
        ...
        if self.model_b_follow_cot_results:
            ...
            fixed_a_answers = [self.get_fixed_cot_and_answer(i)[1] for i in range(len(self.model_a_results))]
        ...
        for intervention_config in self.config.get_intervention_configs():
            ...
            if results_path.exists() and model_b_follow_rollouts is not None:
                ...
                intervention_metrics[run_id] = {'flip_rate': flip_rate, 'accuracy': int_acc, 'omr': int_omr}
                print(f'Intervention {run_id}: Flip Rate = {flip_rate}')
        return ExperimentResults(experiment_name='cot_faithfulness', config={'n_rollouts': self.config.n_rollouts, 'seed': self.config.random_seed}, model_a_accuracy=model_a_acc, model_b_no_cot_accuracy=model_b_no_cot_acc, model_b_follow_cot_accuracy=model_b_follow_acc, omr=omr, mwc=mwc, mww=mww, intervention_results=intervention_metrics)

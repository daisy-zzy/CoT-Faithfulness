...
def analyze_with_stats(baselines_dir, interventions_dir):
    ...
    for path in sorted(interventions_dir.glob("*.jsonl")):
        ...
        if 'baseline' in results:
            ...
            results[name]['vs_baseline'] = paired_ttest(baseline_match, intervention_match)
...
def print_statistical_summary(results):
    ...
    for name in sorted_keys:
        ...
        # Format with CI
        match_str = f"{100*match['mean']:.1f}±{100*(match['ci_high']-match['mean']):.1f}"
        flip_str = f"{100*(1-match['mean']):.1f}±{100*(match['ci_high']-match['mean']):.1f}"
        acc_str = f"{100*acc['mean']:.1f}±{100*(acc['ci_high']-acc['mean']):.1f}"
        mwc_str = f"{100*mwc['mean']:.1f}±{100*(mwc['ci_high']-mwc['mean']):.1f}" if mwc['n'] > 0 else "-"
        mww_str = f"{100*mww['mean']:.1f}±{100*(mww['ci_high']-mww['mean']):.1f}" if mww['n'] > 0 else "-"
        ...
        print(f"{display_name:<25} {match_str:>15} {flip_str:>15} {acc_str:>15} {mwc_str:>12} {mww_str:>12} {p_str:>10}")
...
def plot_with_error_bars(results, output_dir):
    ...
    bars1 = ax.bar(x - width/2, match_means, width, yerr=match_errs, 
                   label='Match Rate', color='tab:blue', alpha=0.8, capsize=3)
    bars2 = ax.bar(x + width/2, flip_means, width, yerr=flip_errs,
                   label='Flip Rate', color='tab:orange', alpha=0.8, capsize=3)
    
    # Add significance markers
    for i, (sig, bar) in enumerate(zip(significance, bars2)):
        if sig and i > 0:  # Skip baseline
            ax.annotate('*', xy=(x[i] + width/2, flip_means[i] + flip_errs[i] + 2),
    ...
    for prefix, color, marker, label in [
        ...
        if ks:
            ax.errorbar(ks, means, yerr=[np.array(means)-np.array(ci_lows), 
    ...
    if 'baseline' in results:
        ...
        ax.axhline(y=baseline_flip, color='gray', linestyle='--', alpha=0.7, label='Baseline')
    ...
    for prefix, color, marker, label in [
        ...
        if ps:
            ax.errorbar(ps, means, yerr=[np.array(means)-np.array(ci_lows),
                                          np.array(ci_highs)-np.array(means)],
                       marker=marker, color=color, label=label, linewidth=2,
                       markersize=8, capsize=4)
    ...
    if 'error_injection' in results:
        ...
        error_path = Path("/home/riyaza/10701-project/outputs/interventions/error_injection.jsonl")
        if error_path.exists():
            ...
            categories = ['Baseline\n(No Intervention)', 'Unmodified CoTs\n(Extraction Failed)', 'Modified CoTs\n(Error Injected)']
            
            # Match rates with bootstrap CI
            baseline_mean, baseline_low, baseline_high = bootstrap_ci(baseline_metrics['match_rates'])
            unmod_mean, unmod_low, unmod_high = bootstrap_ci(unmod_metrics['match_rates'])
            mod_mean, mod_low, mod_high = bootstrap_ci(mod_metrics['match_rates'])
            
            means = [100*baseline_mean, 100*unmod_mean, 100*mod_mean]
            errs_low = [100*(baseline_mean-baseline_low), 100*(unmod_mean-unmod_low), 100*(mod_mean-mod_low)]
            errs_high = [100*(baseline_high-baseline_mean), 100*(unmod_high-unmod_mean), 100*(mod_high-mod_mean)]
            
            colors = ['tab:gray', 'tab:blue', 'tab:orange']
            x = np.arange(len(categories))
            
            bars = ax.bar(x, means, yerr=[errs_low, errs_high], color=colors, 
            ...
            for bar, mean in zip(bars, means):
                ax.annotate(f'{mean:.1f}%', xy=(bar.get_x() + bar.get_width()/2, mean + 5),
            ...
            # Statistical test
            t_stat, p_val = stats.ttest_ind(mod_metrics['match_rates'], baseline_metrics['match_rates'])
            sig_str = "***" if p_val < 0.001 else "**" if p_val < 0.01 else "*" if p_val < 0.05 else ""
            ax.annotate(f'p<0.001{sig_str}', xy=(2, means[2] + errs_high[2] + 8), ha='center', fontsize=10)
    ...
    for group_name, prefix, params in groups:
        ...
        for param in params:
            ...
            if key in results:
                ...
                flip_stats = {'mean': 1-r['match']['mean'], 
...
def main():
    ...
    if 'baseline' in results and 'error_injection' in results:
        ...
        print(f"Relative Increase: {100*(error_flip - baseline_flip)/baseline_flip:.1f}%")
